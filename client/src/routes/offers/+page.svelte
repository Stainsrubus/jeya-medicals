...
<div class="min-h-screen px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8">
  <main class="lg:p-6">
    ...
    {:else}
      ...
      {#if groupedOffers && Object.keys(groupedOffers).length > 0}
            ...
            <div class="flex overflow-x-auto scrollbar-hide pb-4 -mx-4 px-4">
              <div class="flex gap-4 lg:gap-6">
                {#each groupedOffers['flat'] as offer (offer._id)}
                  {#each offer.products as product (product._id)}
                    <div class="">
                      <ProductCard
                        id={product._id}
        ...
        {/if}
      {:else if !isLoading && !error && comboOffers.length === 0}
