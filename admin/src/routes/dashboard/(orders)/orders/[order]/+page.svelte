		...
		<div class="flex justify-between items-center">
			...
			<div class="flex items-center gap-2">
				<!-- <Select.Root
					...
					onValueChange={assignDeliveryAgent}
					disabled={['cancelled', 'delivered'].includes(order.status)}
				>
					<Select.Trigger class="w-[180px] ">
						{assignedDeliveryAgent ? assignedDeliveryAgent.split(' -&- ')[1] : 'Select Agent'}
					</Select.Trigger>
					<Select.Content>
						<Select.Group>
							{#each $deliveryAgentQuery.data?.agents || [] as agent}
								<Select.Item value={`${agent._id} -&- ${agent.name}`} label={agent.name}
									...
									{#if product.suggestions?.length > 0}
										<div class="flex gap-2 flex-wrap">
											{#each product.suggestions as suggestion}
												<span class="text-sm bg-zinc-800 p-2 px-4 rounded-full"
													>{suggestion.name}</span
												>
											{/each}
										</div>
									{:else if product.customSuggestion}
										<p class="text-sm text-muted-foreground mt-2">{product.customSuggestion}</p>
									{:else}
			...
			<Card.Content>
				<div class="space-y-2">
					...
					<div class="flex justify-between">
						...
						<span class="text-muted-foreground line-through">₹{order.platformFee}</span>
					</div>
					<div class="flex justify-between">
						<span>Tax</span>
						<span>₹{order.tax}</span>
