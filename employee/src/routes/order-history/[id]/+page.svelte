    ...
    <div class="border rounded-lg bg-white shadow-md mb-4">
      <div class="flex lg:flex-wrap gap-4  items-start justify-between lg:p-4 p-2  bg-[#F2F4F5]">
        <div>
          ...
          <p class="font-semibold whi  text-[#30363C] lg:text-base text-sm">
            {formatDate(order.createdAt)} <span class="hidden lg:block"> at {formatTime(order.createdAt)}</span> 
          </p>
        </div>
        <div>
          <p class="lg:text-lg text-base text-[#4F585E]">Order For</p>
          <p class=" text-lg"> {order.user.username}</p>
   {order.user.mobile}
        </div>
        <!-- <div>
      ...
      <div class="grid grid-cols-1  lg:gap-4 lg:p-4 border-t">
        <div class=" rounded-lg lg:p-4 p-2">
          <h3 class="text-xl font-bold text-[#30363C] mb-2">Shipping Address</h3>
        <div class="flex  gap-10 <flex-></flex->wrap ">
          <div>
            <p class=" text-lg"><span class="font-medium">
              User:
            </span> {order.user.username}</p>
            <p class=" text-lg">Phone: {order.user.mobile}</p>
           </div>
           <div>
            {#if order.user.HospitalMedicalName}
            <p class="font-medium text-lg">{order.user.HospitalMedicalName}</p>
  {/if}
            <p class="text-lg">{order.user.address.flatNo}, {order.user.address.area}</p>
            <p class="text-lg">{order.user.address.nearbyPlaces}</p>
